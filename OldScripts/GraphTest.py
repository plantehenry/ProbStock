...
def graph_offsets(t: Pred, all_offsets=True, offsets=[], total_diff = False):
    ...
    for off in offsets:
        dates = []
        diffs = []
        print(t.offset_returns[off][-1][1])
        print(t.base_offset_returns[off][-1][1])
        csv_data = []
        ...
        for y1, y2 in zip(t.offset_returns[off][1:], t.base_offset_returns[off][1:]):
            if not total_diff:
                ...
                # add_value_to_second_column("test.csv", diffs[-1])
            date = y1[0]
            dates.append(date)
            all_dates.add(date)
...
def graph_total_changes(t: Pred, all_offsets=True):
    if all_offsets == True:
        offsets = [i for i in range(t.pred_distance)]
    all_dates = set()
    plt.figure(figsize=(10, 6))
