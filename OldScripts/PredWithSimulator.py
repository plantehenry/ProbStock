...
class Pred:
    ...
    def simulate_continous(self):
        ...
        for year in range(self.start_year, self.end_year + 1):
            try:
                year_df = self.df_res[self.df_res['Date'].dt.year == year]
                for pred, asset_change, base_change, date in zip(year_df["Pred"], year_df["Raw_Asset"], year_df["Raw_Baseline"], year_df["Date"]): 
                    try:
